...
public class PaymentVO {
	...
	// private JSON Array available_cards; //결제 수단으로써 사용 허가할 카드사 목록, 지정하지 않으면 모든 카드사
	// 허용
	// 현재 SHINHAN, KB, HYUNDAI, LOTTE, SAMSUNG, NH, BC, HANA, CITI, KAKAOBANK,
	// KAKAOPAY, WOORI, GWANGJU, SUHYUP, SHINHYUP, JEONBUK, JEJU, SC 지원
	// ex) [“HANA”, “BC”]

	// private String payment_method_type; //사용 허가할 결제 수단, 지정하지 않으면 모든 결제 수단 허용,
	// CARD 또는 MONEY 중 하나
	// private Integer install_month; //카드 할부개월, 0~12
	// private JSON Map {String:String} custom_json; //결제 화면에 보여줄 사용자 정의 문구, 카카오페이와
	// 사전 협의 필요
	...
	private String pg_token; // 결제승인 요청을 인증하는 토큰, 사용자 결제 수단 선택 완료 시, approval_url로 redirection해줄 때 pg_token을
								// query string으로 전달
	...
	public String toString() {
		return "PaymentVO [pno=" + pno + ", pay=" + pay + ", uno=" + uno + ", cid=" + cid + ", partner_order_id="
				+ partner_order_id + ", partner_user_id=" + partner_user_id + ", item_name=" + item_name + ", quantity="
				+ quantity + ", total_amount=" + total_amount + ", inner_total_amount=" + inner_total_amount
				+ ", tax_free_amount=" + tax_free_amount + ", approval_url=" + approval_url + ", cancel_url="
				+ cancel_url + ", fail_url=" + fail_url + ", cid_secret=" + cid_secret + ", item_code=" + item_code
				+ ", vat_amount=" + vat_amount + ", tid=" + tid + ", next_redirect_app_url=" + next_redirect_app_url
				...
				+ ios_app_scheme + ", created_at=" + created_at + ", tms_result=" + tms_result + ", pg_token="
				+ pg_token + ", approved_at=" + approved_at + "]";
	}
