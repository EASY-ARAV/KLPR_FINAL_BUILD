...
public class PaymentServiceImpl implements PaymentService {
	...
	public PaymentVO kakaoPayReady(PaymentVO vo) {
		...
		paymentDAO.insert_KakaoPayOrder(vo); // DB에 결제 주문서 생성
		PaymentVO paymentVO = paymentDAO.select_kakaoPayOrder(vo); // 주문번호 가져오기

		if (paymentVO != null) {
			vo.setPartner_order_id(Integer.toString(paymentVO.getPno()));
		}
...
//		params.add("approval_url", "http://www.klper.com/kakaoPaySuccess.do"); // 결제 성공 시 redirect url, 최대 255자
//		params.add("cancel_url", "http://www.klper.com/kakaoPayCancel.jsp"); 		// 결제 취소 시 redirect url, 최대 255자
//		params.add("fail_url", "http://www.klper.com/kakaoPaySuccessFail.jsp"); // 결제 실패 시 redirect url, 최대 255자
		params.add("approval_url", "https://www.klper.com/kakaoPaySuccess.do"); // 결제 성공 시 redirect url, 최대 255자
		params.add("cancel_url", "https://www.klper.com/kakaoPayCancel.jsp"); // 결제 취소 시 redirect url, 최대 255자
		params.add("fail_url", "https://www.klper.com/kakaoPaySuccessFail.jsp"); // 결제 실패 시 redirect url, 최대 255자
	...
	public List<PaymentVO> getChargeList(PaymentVO vo) {
		return paymentDAO.getChargeList(vo);
	}
}